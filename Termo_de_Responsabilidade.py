...
# Função para formatar moeda no padrão brasileiro sem depender de locale
...
def gerar_termos(filtrar_ccusto=None):
    ...
    # Caminho para o modelo (papel timbrado)
    ...
    for ccustos, grupo in grupos:
        ...
        for paragraph in texto_padrao.strip().split('\n'):
            ...
            p.add_run(paragraph.strip().format(
        ...
        for _, bem in grupo_ordenado.iterrows():
            ...
            p_valor = row_cells[4].paragraphs[0]
            p_valor.clear()
            run = p_valor.add_run(valor_formatado)
            run.font.size = Pt(9)
            p_valor.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        ...
        assinatura.add_run(f"{responsavel['responsavel']}\n{responsavel['funcao']} do(a) {ccustos} do CFC")
        assinatura.alignment = WD_ALIGN_PARAGRAPH.CENTER
        for run in assinatura.runs:
            run.font.size = Pt(12)
        ...
        df_para_excel.to_excel(f'planilha_{ccustos}.xlsx', index=False)
