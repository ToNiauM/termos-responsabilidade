# -*- coding: utf-8 -*-
"""
Gera o esqueleto CORRIGIDO do TCC (versão 12) a partir do template oficial
...
Uso:           .venv/bin/python 12_build_esqueleto.py
Saída:         12_TCC_Esqueleto_corrigido.docx
...
OUT = "12_TCC_Esqueleto_corrigido.docx"
FIG = "11_figuras/"
...
blank()
subtitle("Local do estudo e base de dados")
blank()
...
blank()
subtitle("Procedimentos de coleta de dados")
blank()
body_p("A coleta de campo ocorreu em duas etapas. A primeira consistiu em um inventário-piloto "
       ...
       "lotes de sincronização cobrindo 222 bens em uma sessão contínua de trabalho. A segunda "
...
body_p("Em ambas as etapas, o sistema registrou, para cada bem verificado, o local conferido, "
       ...
       "Bens reconferidos em mais de uma data preservam apenas a última marcação temporal, de "
       "modo que os quantitativos diários refletem a posição final da base.")
blank()
subtitle("Análise dos dados")
blank()
body_p("A análise quantitativa empregou Python 3, com as bibliotecas pandas e NumPy para "
       "tratamento dos dados e Matplotlib para visualização. Calcularam-se cinco grupos de "
       "indicadores: (i) produtividade efetiva (bens por hora), obtida pela razão entre a "
       "quantidade de bens conferidos e o tempo efetivo de conferência, e seu recíproco, o "
       "tempo por bem, apurados por dia, por etapa da campanha e para o conjunto; (ii) acurácia "
       "locacional, definida como a proporção de bens ativos cujo local físico conferido "
       "coincidiu com o registro do sistema; (iii) cobertura do inventário, definida como a "
       "proporção de bens ativos conferidos, calculada por quantidade e por valor e "
       "desagregada por classe contábil, pavimento e localização; (iv) divergências de "
       "localização, com identificação dos pares origem-destino mais frequentes e sua "
       "classificação em cinco categorias — granularidade cadastral dentro do mesmo setor, "
       "recolhimento a depósito, almoxarifado ou arquivo, origem em localização lógica, "
       "remanejamento entre setores do mesmo pavimento e remanejamento entre pavimentos —; "
       "(v) bens em situação de baixa ou doação localizados fisicamente, indicativos de "
       "descompasso entre o desfazimento contábil e o físico; e (vi) reconciliação das leituras "
       "com o cadastro patrimonial atualizado após a campanha, por meio de tabela de "
       "correspondência entre as denominações antigas e atuais das localizações, derivada "
       "automaticamente da localização atual dos próprios bens, seguida de carga de validação "
       "das leituras no sistema de gestão patrimonial do órgão, cuja importação foi atômica "
       "(aceitou todas as leituras ou nenhuma) e rejeitou bens inexistentes, datas inválidas e "
       "estados de conservação fora do domínio.")
body_p("Complementarmente, aplicaram-se técnicas de estatística descritiva à caracterização "
       "do acervo: análise da composição por classe contábil, distribuição etária dos bens a "
       "partir da data de entrada e classificação ABC pelo valor atual, na qual os bens são "
       "ordenados de forma decrescente de valor e agrupados nas classes A (80% do valor "
       "acumulado), B (15% seguintes) e C (5% restantes). Essas análises subsidiaram a "
       "priorização das rotas de conferência e a discussão sobre o esforço de controle "
       "proporcional ao risco patrimonial.")
body_p("Para projetar o tempo de inventário do acervo completo a partir da amostra-piloto, "
       ...
       "confiança de 95%. A semente aleatória foi fixada para garantir a reprodutibilidade. A "
       "campanha censitária foi utilizada, na sequência, como validação empírica externa dessa "
       "projeção: o tempo efetivo medido, acrescido do tempo projetado para os bens pendentes "
       "ao ritmo da etapa residual, foi confrontado com o intervalo de confiança simulado. Por "
       "fim, à falta de registro histórico cronometrado do processo manual, o tempo do cenário "
...
blank()
subtitle("Caracterização da base patrimonial")
blank()
...
fig("curva_abc.png",
    ...
    "Fonte: Resultados originais da pesquisa")
blank()
subtitle("Diagnóstico do processo manual (AS-IS)")
blank()
...
caption("Fonte: Resultados originais da pesquisa")
blank()
subtitle("Inventário-piloto: produtividade medida e projeção por simulação")
blank()
...
fig("piloto_montecarlo.png",
    ...
    "Fonte: Resultados originais da pesquisa")
blank()
subtitle("Campanha censitária: validação empírica em escala")
blank()
...
blank()
subtitle("Acurácia locacional e valor diagnóstico das divergências")
blank()
...
caption("Fonte: Resultados originais da pesquisa")
blank()
subtitle("Reestruturação administrativa durante o inventário e reconciliação cadastral")
blank()
...
body_p("O achado tem duas implicações. A primeira é metodológica: a acurácia locacional medida "
       ...
       "órgão, conforme descrito na subseção sobre o pipeline de dados.")
blank()
subtitle("Estado de conservação e desdobramentos administrativos")
blank()
...
blank()
subtitle("Cobertura do inventário e bens pendentes")
blank()
...
body_p("Do ponto de vista econômico, o custo de implantação da solução restringiu-se a horas "
       ...
       "de inventário.")
blank()
subtitle("Pipeline de dados e painéis gerenciais")
blank()
