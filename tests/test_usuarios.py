...
def test_criar_e_buscar(dados):
    ...
    assert u["login"] == "antonio" and u["nome"] == "Antônio Sousa" and u["funcoes"] == ("admin",)
...
def test_criar_valida_login_nome_senha_funcoes(dados):
    ...
    with pytest.raises(db.ErroDeNegocio, match="função válida"):
        usuarios.criar(dados, "fulano", "Fulano", "Senha!234", ["chefe"])     # função inexistente
    with pytest.raises(db.ErroDeNegocio, match="função válida"):
        usuarios.criar(dados, "fulano", "Fulano", "Senha!234", [])            # coleção vazia
    with pytest.raises(db.ErroDeNegocio, match="funções do usuário"):
        usuarios.criar(dados, "fulano", "Fulano", "Senha!234", "operador")    # string em vez de coleção
...
def test_listar_filtra_busca_funcao_e_inativos(dados):
    ...
    assert [u["login"] for u in usuarios.listar(dados, funcao="operador")] == ["beto"]
    assert "senha_hash" not in usuarios.listar(dados)[0]


def test_editar_nome_funcoes_ativo_e_travas(dados):
    ...
    assert u["nome"] == "Beto Silva" and u["funcoes"] == ("inventariante",) and u["login"] == "beto"
    ...
    assert usuarios.por_id(dados, a)["funcoes"] == ("consulta",)
...
def test_elegiveis_comissao(dados):
    ...
    assert [(u["nome"], u["funcoes"]) for u in usuarios.elegiveis_comissao(dados)] == [
        ("Ana", ("admin",)), ("Cris", ("inventariante",))]


def test_usuario_local():
    assert usuarios.USUARIO_LOCAL["id"] is None and usuarios.USUARIO_LOCAL["funcoes"] == ("admin",)
...
def test_criar_admin_cria_ou_redefine(dados):
    ...
    assert usuarios.por_id(dados, uid)["funcoes"] == ("admin",) and usuarios.por_id(dados, uid)["trocar_senha"] == 0
    for _ in range(5):
        with pytest.raises(db.ErroDeNegocio):
...
def test_main_criar_admin(dados, capsys):
    ...
    assert usuarios.por_login(dados, "ze")["funcoes"] == ("admin",)
